...
class Sales extends CI_Controller {
    ...
    public function getAreas(){
        $areas = $this->db->query("select * from tbl_district ")->result();
        echo json_encode($areas);
    }

    public function getAreaWiseCustomer(){
    $data = json_decode($this->input->raw_input_stream);
    $getCustomers =    $this->db->query("
        select
        c.*,
        d.District_Name,
    ...
    order by c.Customer_SlNo desc
        ",[$this->session->userdata('BRANCHid'),$data->area_id])->result();
    ...
    public function getSaleDetails(){
        ...
        $total = 0;
        foreach($saleDetails as $key=>$val){
           $total = $total + ($val->SaleDetails_Rate*$val->SaleDetails_TotalQuantity);
        }
        array_push($saleDetails,$total);
    ...
    public function getSales(){
        ...
        $sales = $this->db->query("
            ...
            left join tbl_employee e on e.Employee_SlNo = sm.employee_id
            left join tbl_brunch br on br.brunch_id = sm.SaleMaster_branchid
            left join tbl_district a on a.District_SlNo = c.area_ID
            where sm.SaleMaster_branchid = '$branchId'
            and sm.Status = 'a'
            $clauses
            order by sm.SaleMaster_SlNo desc
