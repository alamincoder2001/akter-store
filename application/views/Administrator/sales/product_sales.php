...
// 					$CheckSuperAdmin = $this->db->where('UserType','a')->where('User_SlNo',$userID)->get('tbl_user')->row();
...
// 				?>
				...
				<?php //}else{?>
						<!--<div class="col-sm-3">-->
						<!--	<input class="form-control" id="salesDate" type="date" v-model="sales.salesDate"  v-bind:disabled="userType == 'u' ? true : false" readonly/>-->
						<!--</div>-->
				<!--<?php	//}?>-->
			</div>
		</div>
				...
				<div class="widget-main">
					...
					<div class="row">
						<div class="col-sm-5">
							...
							<div class="form-group">
								<label class="col-sm-4 control-label no-padding-right"> Area </label>
								<div class="col-sm-7">
									<v-select v-bind:options="areas" label="District_Name" v-model="selectedArea" v-on:input="areaOnChange"></v-select>
								</div>
								<div class="col-sm-1" style="padding: 0;">
									<a href="<?= base_url('area')?>" class="btn btn-xs btn-danger" style="height: 25px; border: 0; width: 27px; margin-left: -10px;" target="_blank" title="Add New Customer"><i class="fa fa-plus" aria-hidden="true" style="margin-top: 5px;"></i></a>
								</div>
							</div>
							<div class="form-group">
...
<script>
	...
	new Vue({
		...
		methods:{
			async areaOnChange(){
				if(this.selectedArea == null || this.selectedArea.District_SlNo == '0'){
					return false
				}else{
			...
			addToCart(){
				...
				if(cartInd > -1){
					...
					this.cart[cartInd].q = parseFloat(this.cart[cartInd].quantity) + parseFloat(product.quantity);
					if (this.productStock < this.cart[cartInd].q) {
						alert('Stock unavailable');
					}else{
					this.cart[cartInd].quantity = parseFloat(this.cart[cartInd].quantity) + parseFloat(product.quantity);
					this.cart[cartInd].total = parseFloat(this.cart[cartInd].total) + parseFloat(product.quantity * product.salesRate);
					}
