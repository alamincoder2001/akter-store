		...
		<div class="col-md-12">
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'customer' && customers.length > 0 ? '' : 'none'}">
					<label>Area</label>
					<v-select v-bind:options="areas" v-model="selectedArea" label="District_Name" v-on:input="getAreaWiseCustomers"></v-select>
				</div>
						...
						<tbody>
							<template v-for="sale in sales">
								...
								<tr v-for="product in sale.products">
									...
									<td style="text-align:right;">{{ product.product_rate*product.quantity }}</td>
							...
							<td style="text-align:right;">{{ salesTotal.slice(-1).pop() }}</td>
						       </tr>
						</tbody>
					</table>
				</template>
...
<script>
	...
	new Vue({
		...
		methods: {
			async getAreas(){
				  await  axios.get('/get_areas').then(res => {
					this.areas = res.data;
				  })
			},
			...
			getSaleDetails(){
									...
									products: _.chain(sale)
										...
										.map(product => {
											return {
												...
												product_rate:product[0].Product_SellingPrice,
				...
				//  this.total = 	this.products.redure((prev,curr)=>{
				//          return prev*(curr.product_rate*curr.quantity)
				// 			})

					//    let total =  0
					// 	this.products.forEach((item)=>{
					// 		console.log(item.product_rate)
					// 		 total =  total + parseFloat(item.Product_SellingPrice*item.quantity)  
					// 	})
				...
				// this.total = this.sales[0].products.reduce((prev,curr)=>{
